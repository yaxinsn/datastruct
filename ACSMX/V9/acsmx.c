...
int InitNextState(ACSM_STATETABLE* a)
{
    ...
    int j = 0;
    return 0;
    for(i = 0;i<SUBARRAYOFSTATE_COUNT;i++)
    {
        a->__NextState[i] = AC_MALLOC(sizeof(int)*EACH_SUBARRAY_SIZE);
        if(a->__NextState[i] == NULL)
        {
            goto FAIL;
        }
        memset(a->__NextState[i],0,sizeof(int)*EACH_SUBARRAY_SIZE);
    }
    return 0;
FAIL:
    while(j<i)
//    for(j = 0; j < i; j++)
    {
        AC_FREE(a->__NextState[j]);
		a->__NextState[j] = NULL;
        j++;
    }
    return -1;
}
...
int FreeNextState(ACSM_STATETABLE* a)
{
    ...
    for(i = 0;i<SUBARRAYOFSTATE_COUNT;i++)
    {
        AC_FREE(a->__NextState[i]);
		a->__NextState[i] = NULL;
    }
    return 0;

}
int SetNextState(ACSM_STATETABLE* a,unsigned char index, int v)
{
    int array_index = index/EACH_SUBARRAY_SIZE;
    int value_index = index%EACH_SUBARRAY_SIZE;
    if(a->__NextState[array_index])
        a->__NextState[array_index][value_index] =v;
    else
    {
        a->__NextState[array_index] = AC_MALLOC(sizeof(int)*EACH_SUBARRAY_SIZE);

        if(a->__NextState[array_index] == NULL)
            return -1;
        memset(a->__NextState[array_index],-1,sizeof(int)*EACH_SUBARRAY_SIZE);
        a->__NextState[array_index][value_index] =v;

    }
    return 0;
}
int GetNextStateDefault(ACSM_STATETABLE* a, unsigned char index,int default_value)
{

    int array_index = index/EACH_SUBARRAY_SIZE;
    int value_index = index%EACH_SUBARRAY_SIZE;
    if(a->__NextState[array_index] && a->__NextState[array_index][value_index] != -1)
        return a->__NextState[array_index][value_index];
    else
...
AddPatternStates (ACSM_STRUCT * acsm, ACSM_PATTERN * p)
{
...
#else
    next = GetNextStateDefault(&acsm->acsmStateTable[state], *pattern,ACSM_FAIL_STATE);
#endif
...
#else
      if(-1 == SetNextState(&acsm->acsmStateTable[state], *pattern, acsm->acsmNumStates))
      {
        return -1;
      }
#endif
  ...
  if ( -1 == AddMatchListEntry (acsm, state, p))
  {
    LOG("%s:%d -- failed\n",__func__,__LINE__);
    return -1;
  }
...
#else
    s = GetNextStateDefault (&acsm->acsmStateTable[0],i,0);
#endif
      if (s)
      {
        if( -1 == queue_add (queue, s))
        {
...
#else
        if ((s = GetNextStateDefault(&acsm->acsmStateTable[r],i,ACSM_FAIL_STATE)) != ACSM_FAIL_STATE)
#endif
        {
            if( -1 == queue_add (queue, s))
            {
         ...
         while ((next=acsm->acsmStateTable[fs].NextState[i]) ==
                 ACSM_FAIL_STATE)
#else
        while ((next=GetNextStateDefault(&acsm->acsmStateTable[fs],i,fs==0?0:ACSM_FAIL_STATE)) ==
                 ACSM_FAIL_STATE)
#endif
          {
               ...
               mlist  = mlist->next)
          {
              ...
              {
               // FatalError
					LOG("*** Out of memory Initializing Aho Corasick in acsmx.c ****");
		            goto FAIL;
              }
    ...
    while (queue_count (queue) > 0)
    {
      ...
      for (i = 0; i < ALPHABET_SIZE; i++)
      {
        ...
        else
        {
			...
			NextState[i];
#else
            ret = SetNextState(&acsm->acsmStateTable[r], i,
                GetNextStateDefault(&acsm->acsmStateTable[acsm->acsmStateTable[r].FailState],i,0));
            if(-1 == ret)
            {
                goto FAIL;
            }
...
acsmCompile (ACSM_STRUCT * acsm)
{
    ...
    {
        LOG( "%s:%d  acsm->acsmMaxStates %d sizeof (ACSM_STATETABLE) %d \n",__func__,__LINE__,
            acsm->acsmMaxStates,sizeof (ACSM_STATETABLE));
        return -1;
    }
...
//#ifndef REDUCE_NEXTSTATE
    ...
    for (plist = acsm->acsmPatterns; plist != NULL; plist = plist->next)
    {
        if( -1 == AddPatternStates (acsm, plist))
        {
            LOG("%s:%d -- failed\n",__func__,__LINE__);
            goto FAIL;
        }
...
#else
        if(GetNextStateDefault(&acsm->acsmStateTable[0],i,ACSM_FAIL_STATE) == ACSM_FAIL_STATE)
#endif
        {
#ifndef NEXTSTATE_ARRAY
    ...
    if(-1 == Convert_NFA_To_DFA (acsm))
    {
        goto FAIL;
    }

    /*
      LOG ("ACSMX-Max Memory: %d bytes, %d states\n", max_memory,
        acsm->acsmMaxStates);
     */
...
#else
        state = GetNextStateDefault(&StateTable[state],*T,0);
#endif
#if 0
	     if(tol &&state == 0) //V4.2 add it .当发现state =0，则关键字已经不在源的TOP了。 这个代码不正确。
		     return 0;
#endif
...
int acsmPrintSummaryInfo(void)
{
    ...
    LOG("+--[Pattern Matcher:Aho-Corasick Summary]----------------------\n");
    LOG("| Alphabet Size    : %d Chars\n",p->acsmAlphabetSize);
    LOG("| Sizeof State     : %d bytes\n",sizeof(acstate_t));
    LOG("| Storage Format   : %s \n",sf[ p->acsmFormat ]);
    LOG("| Num States       : %d\n",summary.num_states);
    LOG("| Num Transitions  : %d\n",summary.num_transitions);
    LOG("| State Density    : %.1f%%\n",100.0*(double)summary.num_transitions/(summary.num_states*p->acsmAlphabetSize));
    LOG("| Finite Automatum : %s\n", fsa[p->acsmFSA]);
